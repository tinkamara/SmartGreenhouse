...
class TestSoilHumiditySensor(unittest.TestCase):

    def setUp(self):
        self.soil_humidity_sensor = SoilHumiditySensor()

    def test_initial_air_humidity_value(self):
        self.assertEqual(self.soil_humidity_sensor.get_value(), 25)

    def test_lower_value(self):
        self.soil_humidity_sensor.lower_value(5)
        self.assertEqual(self.soil_humidity_sensor.get_value(), 20)

    def test_raise_value(self):
        self.soil_humidity_sensor.raise_value(10)
        self.assertEqual(self.soil_humidity_sensor.get_value(), 35)
    ...
    def test_random_value_change(self):
        ...
        with unittest.mock.patch('random.choice', return_value=2):
            self.soil_humidity_sensor.random_value_change()
        self.assertEqual(self.soil_humidity_sensor.get_value(), 27)
    ...
    def test_random_value_change_with_actual_random(self):
        ...
        original_value = self.soil_humidity_sensor.get_value()
        self.soil_humidity_sensor.random_value_change()
        new_value = self.soil_humidity_sensor.get_value()
